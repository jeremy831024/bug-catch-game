...
> 注：本文件是早期 3D Web 升级方案的技术背景记录。当前主方向已切换为 Godot 沙盒生存方案，团队讨论请优先查看 [godot-sandbox-survival-prd.md](./godot-sandbox-survival-prd.md) 和 [godot-sandbox-survival-architecture.md](./godot-sandbox-survival-architecture.md)。
...
- 模型、贴图、动画、碰撞和玩法逻辑分层解耦，资源替换不影响核心规则。
...
- 树冠、树干、贴图、动画和碰撞分开管理，便于后续逐项替换。
- 后续替换正式三维模型时，只替换外观资源层，不修改树木的逻辑数据、碰撞体和知了停靠点。
...
后续可替换为低多边形 glTF 模型、贴图和骨骼动画。替换时保持角色 id、基础属性、碰撞体和动画状态机不变。

### 8.4 玩家动作设计

第一版动作也采用占位符动画，通过几何体位置、旋转和缩放实现。后续替换正式模型后，使用同一套动作状态名称接入骨骼动画、贴图切换和特效。
...
- 正式模型上线时，替换 `AnimationClip`、贴图和骨骼资源，不改 CaptureSystem、DiggingSystem 和 MovementSystem。
...
- 先保证动作、轮廓、颜色和行为差异清晰，再逐步替换高质量模型、贴图和动画。
- 每种虫的行为状态机、碰撞体和捕捉判定先稳定，资源替换只影响外观层。
...
- 毒虫状态
- 稀有虫出现规则
- 占位符资源保持不变，后续可逐个替换为正式三维模型、贴图和动画
...
- 3D 模型、贴图和动画资产会带来加载和风格一致性问题。
- 移动端性能需要额外优化。

### 16.2 取舍

- 第一版优先用低多边形几何体和简化动画，不急着上复杂模型。
- 第一版不引入重型物理引擎，使用网格碰撞。
- 第一版不做在线排行榜，先保留本地存储。
